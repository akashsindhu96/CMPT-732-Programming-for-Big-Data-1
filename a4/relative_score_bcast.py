...
def main(inputs, output):
    ...
    json_load = jsonfile.map(json.loads)
    ...
    bcast = sc.broadcast(dict(positive_average_rdd.collect()))
    # print(bcast.value)

    commentbysub = json_load.map(lambda c:(c["subreddit"], (c['score'], c["author"])))
    # print(commentbysub.take(10))
    outdata = commentbysub.map(partial(bcast_avg, bcast=bcast)).sortBy(lambda a: a[0], ascending=False)
